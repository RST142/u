...
@CucumberOptions(features = "classpath:EndUser_Forgotpassword", plugin = { "pretty", "html:target/cucumber-html-report",
		"json:target/cucumber.json" }, glue= {"step_defination"},tags = { "@Forgotpassword" })

public class RunCukesEUForgotPassword {
	...
	@BeforeClass
	public void onStart() {
		GlobalUtil.reportfolder="EndUser-Forgot-Password";
	...
	public Object[][] forgotpassword() {
		...
		return utilities.DataUtil.getTestData(xls, "End-User-Forgot-Password","EUForgotpasswordData");
	}
	...
	private void LoginWithoutPhone(Hashtable<String, String> data) throws InterruptedException {
		...
		for (Object[] objects : features) {
			...
			if (objects[0].toString().equals("End-User-Forgot-Password")) {
				if (!DataUtil.checkRunnable(objects[0].toString(), xls, "EUForgotPassword")) {

					throw new SkipException("Skipping the test as runmode is N");
				}
				testdata = new Setterclass();
				testdata.settingdata(data);
				System.out.println("End-User-Forgot-Password");
