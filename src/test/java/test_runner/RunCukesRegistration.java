...
@CucumberOptions(features = "classpath:tenantLogin_Registration", plugin = { "pretty", "html:target/cucumber-html-report",
"json:target/cucumber.json" },glue= {"step_defination"}, tags = { "@SmokeTest" })
public class RunCukesRegistration {
	...
	@BeforeClass 
	public void onStart() {
		GlobalUtil.reportfolder="Tenant-Registration";
	...
	public Object[][] TRegistration() {
		// super.init();

		return utilities.DataUtil.getTestData(xls, "Tenant Registration", "TenantRegistrationData");
	}
	@org.testng.annotations.DataProvider
	public Object[][] regressiontest() {
		// super.init();

		return utilities.DataUtil.getTestData(xls, "Regression for Registration Page", "TenantRegistrationData");
	}
	



	@Test(dataProvider = "TRegistration")
	private void Registration(Hashtable<String, String> data) throws InterruptedException {

		for (Object[] objects : features) {

			 if (objects[0].toString().equals("Tenant Registration")) {
			if (!DataUtil.checkRunnable(objects[0].toString(), xls,"TenantRegistration")) {
	...
	private void regressionRegistration(Hashtable<String, String> data) throws InterruptedException {

		for (Object[] objects : features) {

			if (objects[0].toString().equals("Regression for Registration Page")) {
			if (!DataUtil.checkRunnable(objects[0].toString(), xls,"TenantRegistration")) {
