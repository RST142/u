...
@CucumberOptions(features = "classpath:tenantLogin_Registration", plugin = { "pretty",
		"html:target/cucumber-html-report", "json:target/cucumber.json" },glue= {"step_defination"}, tags = { "@TenantLogin,@Forgotpassword" })
public class RunCukesTanant {
	...
	@BeforeClass
	public void onStart() {
		GlobalUtil.reportfolder = "Tenant-Login-And-FP";
	...
	public Object[][] TenanLogin() {
		// super.init();

		return utilities.DataUtil.getTestData(xls, "Tenant Login", "TenantData");
	}
	...
	public Object[][] forgotpassword() {
		...
		return utilities.DataUtil.getTestData(xls, "Tenant Forgot Password", "TenantData");
	}

	@Test(dataProvider = "TenanLogin")
	private void TenantLogin(Hashtable<String, String> data) throws InterruptedException {

		for (Object[] objects : features) {

			if (objects[0].toString().equals("Tenant Login")) {
				if (!DataUtil.checkRunnable(objects[0].toString(), xls, "Tenant")) {
	...
	private void TenantFogotpassword(Hashtable<String, String> data) throws InterruptedException {
		System.out.println(data.get("URL"));
		for (Object[] objects : features) {

			if (objects[0].toString().equals("Tenant Forgot Password")) {
				if (!DataUtil.isRunnablee(objects[0].toString(), xls)) {
