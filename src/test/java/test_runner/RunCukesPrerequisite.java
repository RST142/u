...
@CucumberOptions(features = "classpath:Prerequisite", 
plugin = { "pretty", "html:target/cucumber-html-report",
"json:target/cucumber.json" },glue= {"step_defination"}, tags = {"@Prerequisite" })
public class RunCukesPrerequisite  {
	...
	@BeforeClass
	public void onStart() {
		GlobalUtil.reportfolder="Prerequisite-End-User";
	...
	public Object[][] PrerequisitewithOC() {
		// super.init();

		return utilities.DataUtil.getTestData(xls, "Prerequisite-End-User","PrerequisiteData");
	}
	
	@Test(dataProvider = "PrerequisitewithOC")
	private void ftuuserwithphone(Hashtable<String, String> data) throws InterruptedException {
		for (Object[] objects : features) {

			if (objects[0].toString().equals("Prerequisite-End-User")) {
				if (!DataUtil.checkRunnable(objects[0].toString(), xls,"Prerequisite")) {

					throw new SkipException("Skipping the test as runmode is N");
				}
				testdata = new Setterclass();
				testdata.settingdata(data);
				System.out.println("Prerequisite-End-User");
