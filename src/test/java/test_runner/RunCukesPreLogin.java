...
@CucumberOptions(features = "classpath:EndUser_PreLogin", plugin = { "pretty", "html:target/cucumber-html-report",
		"json:target/cucumber.json" }, glue = { "step_defination" }, tags = { "@PreLogin-Notifications,@PreLoginNotificationsPopUp" })
public class RunCukesPreLogin {
	...
	@BeforeClass 
	public void onStart() {
		GlobalUtil.reportfolder = "Pre-Login-Notifications";
	...
	public Object[][] PreLoginNotifications() {
		// super.init();

		return utilities.DataUtil.getTestData(xls, "PreLogin-Notifications", "LoginData");
	}
	
	@org.testng.annotations.DataProvider
	public Object[][] PreLoginNotificationPopUp() {
		// super.init();

		return utilities.DataUtil.getTestData(xls, "PreLogin-NotificationsPopUp", "LoginData");
	}
	
	@Test(dataProvider = "PreLoginNotifications")
	private void PreLoginNotification(Hashtable<String, String> data) throws InterruptedException {
		System.out.println(data.get("URL"));
		GlobalUtil.scenarioname = data.get("CombinationName");
		for (Object[] objects : features) {

			if (objects[0].toString().equals("PreLogin-Notifications")) {
	...
	@Test(dataProvider = "PreLoginNotificationPopUp")
	private void PreLoginNotificationPopUp(Hashtable<String, String> data) throws InterruptedException {
		System.out.println(data.get("URL"));
		GlobalUtil.scenarioname = data.get("CombinationName");
		for (Object[] objects : features) {

			if (objects[0].toString().equals("PreLogin-NotificationsPopUp")) {
				if (!DataUtil.checkRunnable(objects[0].toString(), xls, "Login")) {
				...
				testdata = new Setterclass();
				testdata.settingdata(data);
				System.out.println("PreLogin-NotificationsPopUp");
				testNGCucumberRunner.runCucumber(((CucumberFeatureWrapper) objects[0]).getCucumberFeature());
			}}
		}
